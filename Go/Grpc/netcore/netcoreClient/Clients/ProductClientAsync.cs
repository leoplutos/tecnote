...
class ProductClientAsync
{
	// appsettings.json 和 环境变量
	private readonly IConfiguration _config;

	public ProductClientAsync(IConfiguration config)
	{
		_config = config;
	}

	public async Task AsyncClient(bool isWithEtcd)
	{
		Log.Information("AsyncClient 开始");
		...
		{
			// 添加商品
			ProductId pid = await AddProductAsync(client);
		...
		// 检查服务状态
		var _ = await HealthCheckAsyncs(healthClient);

		// 关闭频道
		await channel.ShutdownAsync();

		Log.Information("AsyncClient 结束");
	}

	// 添加商品
	private static async Task<ProductId> AddProductAsync(ProductInfo.ProductInfoClient client)
	{
		...
		ProductId result = await client.addProductAsync(new Product { Name = "Chrome Book 2024", Description = "Add by .NET Core(C#)" }, callOptions);
		Log.Information("[.NET Core(C#)][Client] Add product success. id={id}", result.Value);
		return result;
	}

	// 取得商品
	private static async Task GetProductAsync(ProductInfo.ProductInfoClient client, ProductId pid)
	{
		// 参数可以传递给一个一个调用，也可以组合。使用流畅的语法转换为 CallOptions。
		var callOptions = new CallOptions()
			//.WithCancellationToken(CancellationToken.None)
	...
	//健康检查
	private static async Task<ServingStatus> HealthCheckAsyncs(Health.HealthClient client)
	{
		// HealthCheck检查的服务名为空
		HealthCheckResponse response = await client.CheckAsync(new HealthCheckRequest { Service = "" });
